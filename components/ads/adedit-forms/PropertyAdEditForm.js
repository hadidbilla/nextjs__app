...
function PropertyAdEditForm({ ad, t }) {
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-50">
          ...
          <Form.Row>
            <Col md={6}>
              <Form.Group>
                <Form.Label>{t("land_plot")}:</Form.Label>
                <Form.Control
                  type="text"
                  name="plot_size"
                  placeholder={t("land_plot")}
                  ref={register}
                  disabled={loading}
                  defaultValue={ad?.plot_size}
                />
              </Form.Group>
            </Col>
            <Col md={6}>
...
PropertyAdEditForm.getInitialProps = async () => ({
  namespacesRequired: ["ad-form"],
});
export default withTranslation("ad-form")(PropertyAdEditForm);
