...
export default function TopAdModal({ show, setShow, ad }) {
  ...
  const { paymentMethods, topAdPackages } = useSelector(
  ...
  const onPackageSelect = (id) => {
    setSelectedPackage(topAdPackages.find((item) => item.id == id));
  };

  const onSubmit = async (data) => {
    setLoading(true);

    const res = await dashboardAPI.makeTopAd(data);
    ...
    if (res !== undefined && res.id) {
      ...
      dispatch(
        setActiveSuccessModal({
          active: true,
          message:
            "Your Top Ad request is placed successfully. Please wait for the cofirmation",
        })
      );
    } else {
      // Show Error Modal
    ...
    <Modal show={show} onHide={onHide}>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <Modal.Header closeButton>
          <Modal.Title>Promote "{ad?.ad_title}"</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
            <Form.Label>Top Ad Packages</Form.Label>
            <Form.Control
              as="select"
              name="package"
            ...
            >
              <option value="">Select Top Ad Package</option>
              {topAdPackages &&
                topAdPackages.map((item) => (
          ...
          {selectedMethod && selectedMethod.id && (
            <blockquote className="border border-danger p-2">
              ...
              <h3 className="text-secondary m-0">
                Payable amount: {selectedPackage?.price} BDT
              </h3>
